function [ H, HforX] = Robust_COPA_optimizer( X, R,varargin )
...
params.addParamValue('initX', 'random', @(x) (iscell(x) || ismember(x,{'random','nvecs'})));
...
if (PARFOR_FLAG)
    parfor n = 1:N
        GG{n} = init{n}'*init{n};
        U{n}=sparse(zeros(size_ten(n),R));
        UL{n}=sparse(zeros(size_ten(n),R));
    end
else
    for n = 1:N
        GG{n} = init{n}'*init{n};
...
for itr = 1:1
    for n =1:N
       [init, initX, U, UL, GG,iter]=Robust_fastADMM( X, init, initX, U, UL, n, GG, Constraints,PARFOR_FLAG);
