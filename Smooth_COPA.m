function [fit_gt,U,V,W] = Smooth_COPA( X_gt,X,Omega,R,conv_tol,seed,PARFOR_FLAG,normXgt,normXobs,Constraints,GAP,Basis)
...
while(abs(fit_gt-prev_fit)>conv_tol*prev_fit)
    prev_fit=fit_gt;
    if(PARFOR_FLAG)
        parfor k=1:K
    ...
    fit_gt=calculate_fit(X_gt,Rknew,H,W,V,normXgt,K,PARFOR_FLAG)
    %fit=calculate_fit_obs(X,Rknew,H,W,V,normXobs,K,PARFOR_FLAG,Omega);
     
    % diffnorm = calculate_diffnorm(X_gt,Xclean,Omega,PARFOR_FLAG,normXgt,K)
    tEnd = toc(tStart);
    FIT_TIME(itr,1)=tEnd;
    FIT_TIME(itr,2)=fit_gt;
    if(itr==50)
        break;
    end
end
